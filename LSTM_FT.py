...
def dataframe_principal(simbolo_ativo):
    ...
    #df["{simbolo_ativo}_t3".format(simbolo_ativo = simbolo_ativo)] = ta.ROC(df.Close.values, timeperiod = 4)
    #df["{simbolo_ativo}_t4".format(simbolo_ativo = simbolo_ativo)] = ta.ROC(df.Close.values, timeperiod = 5)
    df = df.drop(['Voltick', 'High', 'Open', 'Low', 'Volume', 'Close'],axis=1)
    '''
    for i in range(len(df[simbolo_ativo])):
        if(df[simbolo_ativo].iloc[i] >= 0):
            df[simbolo_ativo].iloc[i] = 1
    ...
    '''
    df['12ema'] = ta.EMA(df.Close.values, timeperiod = 12)
    df['36ema'] = ta.EMA(df.Close.values, timeperiod = 36)
    
    #df['upperband'], df['middleband'], df['lowerband'] = ta.BBANDS(df.Close.values, timeperiod=5, nbdevup=2, nbdevdn=2, matype=0)
    df['ht_trendline'] = ta.HT_TRENDLINE(df.Close.values)
    ...
    #momentum indicators
    '''
    df['Apo'] = ta.APO(df.Close.values, fastperiod=12, slowperiod=26, matype=0)
    df['ADXR'] = ta.ADXR(df.High.values, df.Low.values, df.Close.values, timeperiod=14)
    df['ADX'] = ta.ADX(df.High.values, df.Low.values, df.Close.values, timeperiod=14)
    df['AROONDOWN'], df['AROONUP'] = ta.AROON(df.High.values, df.Low.values, timeperiod=14)
    df['AROONOSC'] = ta.AROONOSC(df.High.values, df.Low.values, timeperiod=14)
    ...
    df['CMO']= ta.CMO(df.Close.values, timeperiod=14)
    df['DX'] = ta.DX(df.High.values, df.Low.values, df.Close.values, timeperiod=14)
    df['MACD'], df['MACDSIGNAL'], df['MACDHIST'] = ta.MACD(df.Close.values, fastperiod=12, slowperiod=26, signalperiod=9)
    df['MACDfix'], df['MACDSIGNALfix'], df['MACDHISTfix']= ta.MACDFIX(df.Close.values, signalperiod=9)
    df['MFI'] = ta.MFI(df.High.values, df.Low.values, df.Close.values, df.Volume.values, timeperiod=14)
    df['MINUSDM'] = ta.MINUS_DM(df.High.values, df.Low.values, timeperiod=14)
    df['MOM']= ta.MOM(df.Close.values, timeperiod=10)
    df['RSI'] = ta.RSI(df.Close.values, timeperiod=14)
    df['FASTATK'], df['FASTD'] = ta.STOCHF(df.High.values, df.Low.values, df.Close.values, fastk_period=5, fastd_period=3, fastd_matype=0)
    df['FASTATKSI'], df['FASTDSI'] = ta.STOCHRSI(df.Close.values, timeperiod=14, fastk_period=5, fastd_period=3, fastd_matype=0)
    df['ULTOSC']  = ta.ULTOSC(df.High.values, df.Low.values, df.Close.values, timeperiod1=7, timeperiod2=14, timeperiod3=28)
    df['WILLR']  = ta.WILLR(df.High.values, df.Low.values, df.Close.values, timeperiod=14)
    
    #df.Volume.values indicators
    
    df['ADOSC']  = ta.ADOSC(df.High.values, df.Low.values, df.Close.values, df.Volume.values, fastperiod=3, slowperiod=10)
    df['AD']  = ta.AD(df.High.values, df.Low.values, df.Close.values, df.Volume.values)
    df['OBV']  = ta.OBV(df.Close.values, df.Volume.values)
    
    #volatility indicators
    df['NATR']  = ta.NATR(df.High.values, df.Low.values, df.Close.values, timeperiod=14)
    '''
    
    #df['Trima'] = ta.TRIMA(df.Close.values, timeperiod=30)
    #df['TripleEMA'] = ta.T3(df.Close.values, timeperiod=5, vfactor=0)
    #df['SAR'] = ta.SAR(df.High.values, df.Low.values, acceleration=0, maximum=0)
    #df['doubleEMA'] = talib.DEMA(df.Close.values, timeperiod=30)
    #df['kama'] = ta.KAMA(df.Close.values, timeperiod=30)
...
for i in range(len(y_test_real)):
    if(predictions[i] > 0 and y_test_real[i] > 0):
        sucesso+=1
    if(predictions[i] < 0 and y_test_real[i] < 0):
        sucesso+=1
...
def prever_resultado(close_real, fechamento_anterior, previsao, margem):
    lucro = 0
    prejuizo = 0
    falha = 0
    sucesso = 0
...
for i in range(len(y_test_real)):
    ...
    lucro, falha, sucesso, prejuizo, sucesso_compra, falha_compra, sucesso_venda, falha_venda = prever_resultado(y_test_real[i], y_test_real[i-1], predictions[i], variacao)
